...
class ProfitScanner:
    ...
    async def detect_flag_pennant(self, symbol, data):
        """Обнаруживает формации флаг/вымпел"""
        try:
            if len(data) < 30:
                return None
            ...
            # Проверка тренда перед формацией
            price_5 = data['close'].iloc[-5]
            price_20 = data['close'].iloc[-20]
            trend_up = price_5 > price_20 * 1.03  # 3% рост
            
            # Проверка консолидации
            high_consolidation = data['high'].iloc[-10:].max()
            low_consolidation = data['low'].iloc[-10:].min()
            consolidation_range = (high_consolidation - low_consolidation) / low_consolidation
            
            if trend_up and consolidation_range < 0.02:  # Консолидация < 2%
                entry = high_consolidation * 1.003
                stop_loss = low_consolidation * 0.995
                
                tp1 = entry + (entry - stop_loss) * 1.5
                tp2 = entry + (entry - stop_loss) * 2.5
                tp3 = entry + (entry - stop_loss) * 3.5
            ...
            logger.error(f"Flag detection error for {symbol}: {e}")
        return None
        ...
        try:
            ...
            for symbol in symbols:
                try:
                    ...
                    flag = await self.scanner.detect_flag_pennant(symbol, data)
                    if flag:
                        setups.append(flag)
